...
def validate_workflows(root: Path) -> tuple[int, list[str]]:
    ...
    for path in files:
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if expected is None:
                errors.append(f"{path.relative_to(root)}:{line_number}: {action} is not allowlisted")
            elif revision != expected:
                errors.append(f"{path.relative_to(root)}:{line_number}: {action} drifted from reviewed SHA")
            if not version_comment:
                errors.append(f"{path.relative_to(root)}:{line_number}: pinned action needs a version comment")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        ...
        return 2
    references, errors = validate_workflows(Path(argv[1]).resolve())
    if errors:
        print("GitHub Actions reference validation failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print(f"PASS GitHub Actions: {references} reviewed external reference(s) pinned")
    return 0
